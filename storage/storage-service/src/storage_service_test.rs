    ...
    fn test_simple_storage_service(blocks in arb_blocks_to_commit().no_shrink()) {
        ...
        for (txns_to_commit, ledger_info_with_sigs) in &blocks {
            ...
            txns_to_commit.iter().for_each(|txn_to_commit| {
                account_states.extend(txn_to_commit.state_store_updates().clone())
            });
            ...
            let account_states_returned = account_states
                .keys()
                .map(|address| client.get_account_state_with_proof_by_version(address.clone(), version - 1).unwrap())
                .collect::<Vec<_>>();
