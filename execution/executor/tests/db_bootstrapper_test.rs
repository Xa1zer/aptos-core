...
fn get_balance(account: &AccountAddress, db: &DbReaderWriter) -> u64 {
    let account_state_blob = db
        .reader
        .get_latest_value(StateStoreKey::AccountAddressKey(*account))
        .unwrap()
        .unwrap();
...
fn get_configuration(db: &DbReaderWriter) -> ConfigurationResource {
    let config_blob = db
        .reader
        .get_latest_value(StateStoreKey::AccountAddressKey(config_address()))
        .unwrap()
        .unwrap();
...
fn restore_state_to_db(
    db: &Arc<AptosDB>,
    accounts: Vec<(HashValue, StateStoreValue)>,
    proof: SparseMerkleRangeProof,
    root_hash: HashValue,
    version: Version,
) {
